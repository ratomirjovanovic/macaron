...
def test_macaron_app_methods():
    ...
    try:
        ...
        # Mock os.geteuid for Windows compatibility
        if platform.system() == "Windows":
            with unittest.mock.patch.object(os, 'geteuid', return_value=0, create=True):
                return _test_app_creation(main, tk)
        else:
...
def _test_app_creation(main, tk):
    """Helper function to test app creation"""
    root = tk.Tk()
...
def test_security_features():
    ...
    try:
        ...
        if platform.system() == "Windows":
            with unittest.mock.patch.object(os, 'geteuid', return_value=0, create=True):
                return _test_security_validation(main)
        else:
            with unittest.mock.patch('os.geteuid', return_value=0):
                return _test_security_validation(main)
...
def _test_security_validation(main):
    """Helper function to test security features"""
    import tkinter as tk
    root = tk.Tk()
    root.withdraw()
...
def test_installer_script():
    ...
    try:
        with open(installer_file, 'r', encoding='utf-8', errors='ignore') as f:
            content = f.read()
...
def test_file_structure():
    ...
    for script in old_scripts:
        if not os.path.exists(script):
            ...
            print(f"⚠ Old script still exists: {script} (should be removed)")
...
def test_integration_features():
    ...
    try:
        ...
        if platform.system() == "Windows":
            with unittest.mock.patch.object(os, 'geteuid', return_value=0, create=True):
                return _test_integration_methods(main)
        else:
            with unittest.mock.patch('os.geteuid', return_value=0):
                return _test_integration_methods(main)
...
def _test_integration_methods(main):
    """Helper function to test integration methods"""
    import tkinter as tk
    root = tk.Tk()
    root.withdraw()
